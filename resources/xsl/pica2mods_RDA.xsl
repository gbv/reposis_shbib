...
<xsl:stylesheet version="1.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:p="info:srw/schema/5/picaXML-v1.0" xmlns:mods="http://www.loc.gov/mods/v3"
  ...
  <xsl:template match="/p:record" mode="RDA">
    ...
    <mods:recordInfo>
      ...
      <mods:recordOrigin><xsl:value-of select="normalize-space(concat('Converted from PICA to MODS using ',$XSL_VERSION_RDA))" /></mods:recordOrigin>
    </mods:recordInfo>
 ...
 <xsl:if test="not($pica0500_2='v')">
      ...
      <xsl:variable name="query">
        <xsl:choose>
          <xsl:when test="$ppnA"><xsl:value-of select="concat('sru-gvk:pica.ppn=', $ppnA)" /></xsl:when>
          <xsl:when test="$zdbA"><xsl:value-of select="concat('sru-gvk:pica.zdb=', $zdbA)" /></xsl:when>
        </xsl:choose>
      </xsl:variable>
    ...
    <!--033J = 4033 Druckernormadaten, aber kein Ort angegeben (müsste aus GND gelesen werden) MODS unterstützt keine authorityURIs für Verlage deshalb 033A verwenden , RDA: Drucker-/Verlegernormdaten oben erledigt in 3010/3110 mit entspr. Rollenbezeichnung -->
